...
class PipelineAPI:
    ...
    def get(self, path, data = None, headers = None):
            # If data exists, convert it to query string and append to path
        if data:
            # Start with ? if path doesn't already have one
            if '?' not in path:
                path += '?'
            else:
                # If path already has parameters, add & unless it ends with ?
                if not path.endswith('?'):
                    path += '&'
            
            # Build query string from data dictionary
            query_params = []
            for key, value in data.items():
                if value is not None:  # Skip None values
                    query_params.append(f"{key}={value}")
            
            # Append the query string to path
...
class PipelineCluster(PipelineService):
    ...
    def __get_current_parallel_tasks(self, cluster_id) -> int:
        max_dop = 0
        for base_run in self.workspace.jobs.list_runs(active_only=True):#run_type=RunType.JOB_RUN, 
...
process_functions["{field.name}"] = process_{field.name}
    ...
    def __get_last_run(self):
        ...
        if last_run:
            ...
            if "state" in last_run and "result_state" in last_run["state"]:
