...
class TableService(PipelineSpark):
    ...
    def __init_state(self):
        ...
        if "last_vacuum_time" in self.state and self.state["last_vacuum_time"] is not None:
            self.last_vacuum_time = datetime.strptime(self.state["last_vacuum_time"], '%Y-%m-%d %H:%M:%S')
        if "last_optimize_time" in self.state and self.state["last_optimize_time"] is not None:
