...
class Pipeline(PipelineCluster):
    ...
    def repair_run(self, job_name:str, params = None, timeout = 3600):
        ...
        if job_id:#self.last_run.job_run_id:# and self.last_run.job_run_result in ("FAILED", "TIMEDOUT", "CANCELED"):
            ...
            if base_runs:
                last_base_run: Run = next(base_runs, None)
                if last_base_run:
                    last_run: Run = self.workspace.jobs.get_run(last_base_run.run_id)
                    if last_run and last_run.result_state in (RunResultState.FAILED, RunResultState.TIMEDOUT, RunResultState.CANCELED):
                        self.logs.log("operations", { "operation": f're-run job:{job_name}' })
                        wait_run = self.workspace.jobs.repair_run(last_run.job_run_id, latest_repair_id=last_run.repair_history[-1].id, job_parameters=params, rerun_all_failed_tasks=True)
