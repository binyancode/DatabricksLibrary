...
class TableService(PipelineSpark):
    ...
    def __write_state(self):
        self.__lock.acquire()
        try:
            self.state["last_vacuum_time"] = self.last_vacuum_time.strftime('%Y-%m-%d %H:%M:%S')
            self.state["last_optimize_time"] = self.last_optimize_time.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def __init__(self, table_name, state_path):
        self.table_name = table_name
        self.state_path = state_path
        self.state_file = os.path.join(state_path, f"TableService/{table_name}.json")
        self.__init_state()
